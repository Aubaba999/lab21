...
#include<string>
#include<cstdio>
HWND textfield , button, Firstint,SecondInt;
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT Message, WPARAM wParam, LPARAM lParam) {
	switch(Message) {
				...
				Firstint = CreateWindow("EDIT","",
					WS_BORDER|WS_VISIBLE | WS_CHILD,
				30,50,150,20,hwnd,NULL,NULL,NULL);
				SecondInt= CreateWindow("EDIT","",
					WS_BORDER|WS_VISIBLE | WS_CHILD,
		...
		case WM_COMMAND:
				switch(LOWORD(wParam)){
					...
					firstint = GetWindowText(Firstint,&textSaved1[0],20);
					secondint= GetWindowText(SecondInt,&textSaved2[0],20);
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
	...
	wc.hbrBackground = CreateSolidBrush(RGB(0, 255, 0)); // สีเขียว
